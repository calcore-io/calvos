...
/*                           calvOS Project                                   */
/*============================================================================*/
/** \file		USER_comgen_CAN_DCM_RR_hal.c                                  */
...
#define CAN_DCM_RR_CONFIRM_MSG_ID		kTrue

#if CAN_DCM_RR_CONFIRM_MSG_ID==kTrue
uint32_t can_DCM_RR_HALgetTxdMsgId(void);
#endif
 ...
 * ===========================================================================*/
CalvosError can_DCM_RR_HALtransmitMsg(const CANtxMsgStaticData* msg_info){
...
void can_DCM_RR_HALconfirmTxMsg(void){
	...
	txd_msg_id = can_DCM_RR_HALgetTxdMsgId();
	// Confirm TX message if ID matches
	can_commonConfirmTxMsg(can_DCM_RR_transmittingMsg, kTrue, txd_msg_id);
#else
	can_commonConfirmTxMsg(can_DCM_RR_transmittingMsg, kFalse, NULL);
#endif
}
 ...
 * ===========================================================================*/
void can_DCM_RR_HALreceiveMsg(uint32_t msg_id, uint8_t* data_in, uint8_t data_len){

	#warning "This function shall be called in user's CAN HAL rx ISR. Remove this line when done".
	// Call RX processor function
	can_DCM_RR_processRxMessage(msg_id, data_in, data_len);
}
