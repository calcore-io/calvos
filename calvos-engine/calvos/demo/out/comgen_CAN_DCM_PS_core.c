 ...
 * ===========================================================================*/
void can_task_20ms_DCM_PS_rxProcess(void){
	...
	for(uint32_t i = 0u; i < kCAN_DCM_PS_nOfRxMsgs; i++){
		// Check if message has a timeout defined
		if(can_DCM_PS_rxMsgStaticData[i].timeout > 0u){
			// Only increment timer if message hasn't timed out
			CALVOS_CRITICAL_ENTER();
			if(!can_DCM_PS_rxMsgStaticData[i].dyn->timedout){
				CALVOS_CRITICAL_EXIT();
				// Increment timer (up-counter since they are initialized with zero)
				can_DCM_PS_rxMsgStaticData[i].dyn->timeout_timer++;
				if(can_DCM_PS_rxMsgStaticData[i].dyn->timeout_timer \
				>= can_DCM_PS_rxMsgStaticData[i].timeout){
					// Set timeout flag
					CALVOS_CRITICAL_ENTER();
					can_DCM_PS_rxMsgStaticData[i].dyn->timedout = kTrue;
					CALVOS_CRITICAL_EXIT();
					// Call timeout callback if not NULL
					if(can_DCM_PS_rxMsgStaticData[i].timeout_callback != NULL){
						(can_DCM_PS_rxMsgStaticData[i].timeout_callback)();
					}
					// Reset timer
					can_DCM_PS_rxMsgStaticData[i].dyn->timeout_timer = 0u;
				}
			}else{
				CALVOS_CRITICAL_EXIT();
			}
